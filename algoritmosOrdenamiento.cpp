...
int main(){
    ...
    //Generación de un arreglo con los tamaños de arreglo a probar, i.e., [10,20,30,....,1000]
    
    generarArregloTamanios(arregloTamanios);
    
    //Cálculo de los tiempos de ejecución.
    //Se usan 3 ciclos anidados. El índice j indica el número de tamaño de arreglo que se ordena, el índice i indica 
    //el número de prueba que se realiza y el índice k indica el algoritmo que se utiliza para ordenar.
    ...
    for(int j=0; j<NUM_TAMANIOS; j++){
        ...
        for(int i=0; i<NUM_PRUEBAS; i++){
            ...
            //Para la primera prueba del primer tamaño de arreglo se imprime en pantalla el arreglo generado, el cual se va 
            //a utilizar para verificar que todos los algoritmos ordenen de manera correcta.
            
            if(i==0 && j==0){
                cout<<endl;
                cout<<"Arreglo aleatorio de prueba:"<<endl;
                imprimirArreglo(arregloAleatorio, arregloTamanios[j]);
                cout<<endl;
            }
            ...
            for(int k=0; k<NUM_ALGORITMOS; k++){
                ...
                //Para cada algoritmo, para la primera prueba del primer tamaño de arreglo se imprime en pantalla el resultado
                //del ordenamiento para para verificar que todos los algoritmos hayan ordenado de manera correcta.
                
                if(i==0 && j==0){
                    cout<<"Resultado del ordenamiento ";
                    if(k%2==0) cout<<"por ";
                    cout<<arregloNombres[k]<<":"<<endl;
                    imprimirArreglo(arregloCopia, arregloTamanios[j]);
                    cout<<endl;
                }
            }
        }
...
void SHELL_SORT(int s[], int n){
    int gap, numeroGaps, j, temp;
    int* gaps;
    
    //Cálculo del tamaño del arreglo de los gaps.
    ...
    //Inicialización del arreglo de los gaps.
    
    gaps=new int[numeroGaps];

    gaps[0]=n/2;
    
    for(int i=1; i<numeroGaps; i++){
        gaps[i]=gaps[i-1]/2;
    }
    
    //Una vez que se tiene el arreglo de los gaps, se procede a ordenar el arreglo aleatorio.
    
    for(int k=0; k<numeroGaps; k++){
        
        for(int i=gaps[k]; i<n; i++){
            temp=s[i];
            j=i;
            
            while(j>=gaps[k] && s[j-gaps[k]]>temp){
                s[j]=s[j-gaps[k]];
                j=j-gaps[k];
            }
            s[j]=temp;
